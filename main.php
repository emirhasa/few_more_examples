<?php 
require_once TRAV_INC_DIR . '/lib/payment/paypal.php';
// require_once TRAV_INC_DIR . '/lib/payment/woocommerce.php';
	...
	function trav_process_payment( $payment_data ) { // $payment_data = array('item_name', 'item_number', 'item_desc', 'item_qty', 'item_price', 'item_total_price', 'grand_total', 'status', 'return_url', 'cancel_url', 'deposit_rate')
		...
		if ( trav_is_paypal_enabled() ) {
			...
			if ( empty( $trav_options['acc_pay_paypal_api_username'] ) || empty( $trav_options['acc_pay_paypal_api_password'] ) || empty( $trav_options['acc_pay_paypal_api_signature'] ) ) {
				echo '<div class="alert alert-error">Please check site paypal setting. <a href="' . admin_url( 'themes.php?page=Travelo' ) . '">' . admin_url( 'themes.php?page=Travelo' ) . '</a><span class="close"></span></div>';
				exit;
			}
...
if ( ! function_exists( 'trav_is_woo_enabled' ) ) {
	function trav_is_woo_enabled() {
		...
		if ( ! empty( $trav_options['acc_pay_woocommerce'] ) && is_plugin_active( 'woocommerce/woocommerce.php' ) && is_plugin_active( 'travelo-woocommerce-gateway/travelo-woocommerce-gateway.php' ) ) {
			return true;
		} else {
			return false;
		}
